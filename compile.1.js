...
function writeMessage(ctx, options) {
    ...
    if (!options.noRead) {
        code += 'const ' + name + '_read = function (pbf, end) {\n';
        code += '    return pbf.readFields(' + name + '_readField, ' + compileDest(ctx) + ', end);\n';
        code += '};\n';
        code += 'const ' + name + '_readField = function (tag, obj, pbf) {\n';
    ...
    if (!options.noWrite) {
        code += 'const ' + name + '_write = function (obj, pbf) {\n';
        numRepeated = 0;
...
function compileFieldRead(ctx, field) {
    ...
    if (type) {
        if (type._proto.fields) return type._name + '_read(pbf, pbf.readVarint() + pbf.pos)';
...
function compileFieldWrite(ctx, field, name) {
    ...
    var type = getType(ctx, field);
    if (type) {
        if (type._proto.fields) return prefix + 'Message(' + field.tag + ', ' + type._name + '_write, ' + name + ')';
