...
function writeMessage(ctx, options) {
    ...
    if (!options.noRead || !options.noWrite) {
        code += (!ctx._root ? '  ' : '') + compileExport(ctx, options) + ' {\n';
    }
...
function writeEnum(ctx, options) {
    var str = JSON.stringify(ctx._proto.values, null, 4);
    if (!ctx._root) {
        str = str.substring(0, str.length - 1);
        str += '  }';
    }
    return '\n' + (!ctx._root ? '  ' : '') + compileExport(ctx, options) + ' ' +
        str + ',\n';
}

function compileExport(ctx, options) {
    var exportsVar = options.exports || 'exports';
    return (ctx._root ? 'export const ' + ctx._name + ' = ' : ctx._name + ': ');
}
